...
- `.venv/` — Python virtuella miljöer
- `node_modules/` — npm-beroenden
- `models/` — kompilerad ONNX-modellutdata (stora binära filer, genererade av del 10)
- `cache_dir/` — Hugging Face modellnedladdningscache
- `.olive-cache/` — Microsoft Olive arbetskatalog
- `samples/audio/*.wav` — genererade ljudexempel (nygenereras via `python samples/audio/generate_samples.py`)
- Standard Python byggartefakter (`__pycache__/`, `*.egg-info/`, `dist/`, etc.)
